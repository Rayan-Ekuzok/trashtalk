  ...
  methods: {
    async chargerTout() {
      const [e, c, t] = await Promise.all([
        fetch('https://fellous.alwaysdata.net/emplacement').then(r => r.json()),
        //fetch('http://localhost:3000/emplacement').then(r => r.json()),
        fetch('https://fellous.alwaysdata.net/contenant').then(r => r.json()),
        //fetch('http://localhost:3000/contenant').then(r => r.json()),
        fetch('https://fellous.alwaysdata.net/type_dechet').then(r => r.json())
    ...
    async ajouterEmp() {
      ...
      try {
        const res  = await fetch('https://fellous.alwaysdata.net/emplacement/ajouter', { method: 'POST', headers: this.$auth(), body: JSON.stringify({ libelle, code_postal: cp, latitude: parseFloat(lat), longitude: parseFloat(lng) }) })
        //const res  = await fetch('http://localhost:3000/type_dechet', { method: 'POST', headers: this.$auth(), body: JSON.stringify({ libelle, code_postal: cp, latitude: parseFloat(lat), longitude: parseFloat(lng) }) })
    ...
    async ajouterCont() {
      ...
      try {
        const res  = await fetch('https://fellous.alwaysdata.net/contenant/ajouter', { method: 'POST', headers: this.$auth(), body: JSON.stringify({ capacite_kg: parseInt(cap), poids_actuel_kg: parseFloat(poids) || 0, Id_type_dechet: parseInt(typeId), Id_emplacement: parseInt(empId), scelle: scelle ? 1 : 0 }) })
        //const res  = await fetch('http://localhost:3000/type_dechet', { method: 'POST', headers: this.$auth(), body: JSON.stringify({ capacite_kg: parseInt(cap), poids_actuel_kg: parseFloat(poids) || 0, Id_type_dechet: parseInt(typeId), Id_emplacement: parseInt(empId), scelle: scelle ? 1 : 0 }) })
    ...
    async assigner() {
      ...
      try {
        const res  = await fetch('https://fellous.alwaysdata.net/contenant/assigner', { method: 'POST', headers: this.$auth(), body: JSON.stringify({ Id_contenant: parseInt(contId), Id_emplacement: parseInt(empId) }) })
