...
export default {
  ...
  methods: {
    ...
    async envoyerSignalement() {
      ...
      this.signalementMsg = null
      try {
        const res  = await fetch('https://fellous.alwaysdata.net/signalement/creer', {
    ...
    async chargerDonnees() {
      const [e, c, t] = await Promise.all([
        ...
        fetch('https://fellous.alwaysdata.net/contenant', { headers: this.$auth() }).then(r => r.json()),
    ...
    async remplir(id) {
      const res  = await fetch('https://fellous.alwaysdata.net/remplirpoubelle', {
